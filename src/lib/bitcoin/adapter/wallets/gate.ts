...
export class GateBitcoinWallet implements BitcoinWallet {
  name: string;
  ...
  get provider() {
    return window.gatewallet?.bitcoin;
  }
  ...
  connect = async (): Promise<void> => {
    ...
    try {
      ...
      const publicKey = await (window as any)?.gatewallet?.bitcoin?.getPublicKey();
      debugger
  ...
  signMessage = async (msg: string): Promise<string> => {
    ...
    return await this.provider.signMessage(msg, "utf8");
  };
